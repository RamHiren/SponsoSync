        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {events.map((event) => (
            <div key={event._id} className="bg-white p-4 shadow-md rounded-lg">
              ...
              <p className="text-gray-600">Audience Count: {event.audienceCount}</p>
              <p className="text-gray-600">Organizer: {event.eventOrganizer}</p>
              <p className="text-gray-600">Description: {event.eventDescription}</p>
              <p className="text-gray-600">Location: {event.location?.address}, {event.location?.city}, {event.location?.state}</p>
              <p className={`text-${event.isVerified ? "green" : "red"}-500 font-semibold`}>
                {event.isVerified ? "Verified" : "Not Verified"}
              </p>
              <img src={event.imageUrl} alt={event.eventname} className="w-full h-40 object-cover rounded-lg mt-2" />
